...
describe("grep handler behavior", () => {
  it("matches strings, regexes, and exposes editable tags", async () => {
    ...
    const tag = result.value.match(/([A-Z]+)\|const alpha/)?.[1]

    assert.equal(result.ok, true)
    assert.match(result.value, /# .*sample\.js/)
    
    const edit = await handlers.edit({ begin: tag, endExclusive: tagsOf(result.value)[1], content: "const alpha = 3" })
